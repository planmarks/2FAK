...
// Copy a byte string of up to `max` bytes; returns the actual length via `outlen`.
// Used for pinUvAuthParam / pinHashEnc which are 16 bytes for protocol 1 and 32 for 2.
uint8_t parse_var_byte_string(CborValue * map, uint8_t * dst, unsigned int max, unsigned int * outlen)
{
    size_t sz = max;
    int ret;
    *outlen = 0;
...
uint8_t ctap_parse_hmac_secret(CborValue * val, CTAP_hmac_secret * hs)
{
    ...
    hs->pinProtocol = 1;   // default when the (optional) protocol field is absent
    int key;
    int ret;
    unsigned int i;
    CborValue map;
    ...
    for (i = 0; i < map_length; i++)
    {
        ...
        switch(key)
        {
            ...
            case EXT_HMAC_SECRET_SALT_ENC:
                ...
                // v1: 32 (one salt) or 64 (two). v2: prepends a 16-byte IV -> 48 or 80.
                if ((salt_len != 32 && salt_len != 64 && salt_len != 48 && salt_len != 80)
            ...
            case EXT_HMAC_SECRET_SALT_AUTH:
                ...
                parsed_count++;
            break;
            case EXT_HMAC_SECRET_PIN_PROTOCOL:
                ret = cbor_value_get_int_checked(&map, &hs->pinProtocol);
                check_ret(ret);
            break;
        }
...
uint8_t ctap_parse_extensions(CborValue * val, CTAP_extensions * ext)
{
    ...
    for (i = 0; i < map_length; i++)
    {
        ...
        if (strncmp(key, "hmac-secret",11) == 0)
        {
            ...
            {
                printf2(TAG_ERR, "error: hmac-secret extension is the wrong type\r\n");
                return CTAP2_ERR_INVALID_CBOR_TYPE;
            }
        }
    ...
    for (i = 0; i < map_length; i++)
    {
        ...
        switch(key)
        {
            ...
            case MC_pinAuth: {
                ...
                ret = parse_var_byte_string(&map, MC->pinAuth, 32, &MC->pinAuthLength);
                if (ret != 0)
                {
                    MC->pinAuthLength = 0;   // be lenient; verification will reject it
                    ret = 0;
                }
                MC->pinAuthPresent = 1;
                break;
    ...
    for (i = 0; i < map_length; i++)
    {
        ...
        switch(key)
        {
            ...
            case GA_pinAuth: {
                ...
                ret = parse_var_byte_string(&map, GA->pinAuth, 32, &GA->pinAuthLength);
                if (ret != 0)
                {
                    GA->pinAuthLength = 0;   // be lenient; verification will reject it
                    ret = 0;
                }
    ...
    for (i = 0; i < map_length; i++)
    {
        ...
        switch(key)
        {
            ...
            case CP_pinAuth:
                ...
                ret = parse_var_byte_string(&map, CP->pinAuth, 32, &CP->pinAuthLength);
                check_retr(ret);
                CP->pinAuthPresent = 1;
                break;
            case CP_newPinEnc:
            ...
            case CP_pinHashEnc: {
                ...
                unsigned int hlen = 0;
                ret = parse_var_byte_string(&map, CP->pinHashEnc, 32, &hlen);
                check_retr(ret);
                CP->pinHashEncSize = (int)hlen;
            ...
            case CP_getRetries:
                ...
                break;
            case CP_permissions:
                printf1(TAG_CP,"CP_permissions\n");
                if (cbor_value_get_type(&map) != CborIntegerType)
                {
                    return CTAP2_ERR_INVALID_CBOR_TYPE;
                }
                ret = cbor_value_get_int_checked(&map, &CP->permissions);
                check_ret(ret);
                CP->permissionsPresent = 1;
                break;
            case CP_rpId: {
                printf1(TAG_CP,"CP_rpId\n");
                if (cbor_value_get_type(&map) != CborTextStringType)
                {
                    return CTAP2_ERR_INVALID_CBOR_TYPE;
                }
                uint8_t rpbuf[DOMAIN_NAME_MAX_SIZE + 1];
                size_t rpsz = DOMAIN_NAME_MAX_SIZE;
                ret = cbor_value_copy_text_string(&map, (char*)rpbuf, &rpsz, NULL);
                if (ret == CborErrorOutOfMemory) return CTAP2_ERR_LIMIT_EXCEEDED;
                check_ret(ret);
                crypto_sha256_init();
                crypto_sha256_update(rpbuf, rpsz);
                crypto_sha256_final(CP->rpIdHash);
                CP->rpIdPresent = 1;
                break;
            }
            default:
