...
static void device_migrate(){
  ...
  /* 2FAK: respect an already-provisioned attestation page (factory-injected muru
     cert/key). Only run the migration/fallback when the page is not configured yet,
     so provisioning is not overwritten. */
...
void device_init()
{
    ...
    // Probe the ATECC608B before state is loaded. If present and provisioned it
    // supplies the hardware root used in crypto_load_master_secret(); otherwise it
    // stays dormant and the software master secret is used (no behaviour change).
    atecc_init();
