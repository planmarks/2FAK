...
// ATECC608B hardware root. Only available in the STM32L432 device build; host/test
// builds get inert stubs so the software master secret is used unchanged.
#ifdef STM32L432xx
#include "atecc.h"
#else
static int atecc_ready(void) { return 0; }
static int atecc_kdf(const uint8_t *s, uint8_t *o, uint32_t n) { (void)s; (void)o; (void)n; return 0; }
#endif
...
#error "need more key bytes"
    ...
    // Hardware root (ATECC608B): if a provisioned secure element is present, derive
    // the working master secret from the chip's unreadable root, salted with the
    // per-device flash key. The flash value still drives reset semantics (wiping it
    // changes the salt and therefore every derived credential key). On any unit
    // without a provisioned chip, atecc_ready() is false and we keep the flash
    // master unchanged, so behaviour is identical to the software-only build.
    if (atecc_ready())
    {
        uint8_t derived[64];
        if (atecc_kdf(master_secret, derived, sizeof(derived)))
        {
            memmove(master_secret, derived, sizeof(master_secret));
        }
